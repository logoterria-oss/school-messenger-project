...
export const ChatSidebar = ({ userRole, userName, userId, chats, allUsers = [], selectedChat, selectedTopic, groupTopics, onSelectChat, onTopicSelect, onLogout, onOpenProfile, onOpenSettings, onOpenUsers, onAddStudent, onAddParent, onAddTeacher, onCreateGroup, onAddAdmin, onArchiveChat }: ChatSidebarProps) => {
  ...
  return (
    <div className="w-full md:w-[380px] bg-card/50 backdrop-blur-sm border-r border-border/60 flex flex-col" style={{ height: 'var(--app-height, 100dvh)' }}>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground/70 leading-tight">
                {userRole === 'admin' && `${userName || 'Администратор'} (${userId === 'admin' ? 'руководитель' : 'админ'})`}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button type="button" className="h-9 w-9 flex items-center justify-center rounded-lg text-muted-foreground hover:bg-accent flex-shrink-0">
